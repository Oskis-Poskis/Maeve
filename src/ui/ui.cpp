...
namespace UI
{
    ...
    {
        {
            ...
            if (Title == subMenus[mainMenu.ActiveSubMenu].Title)
            {
                ...
                if (Title == "Scene Manager")
                {
                    ...
                    {
                        float distance  = ((NumItemDistanceShade - glm::abs(ActiveSubMenu - i)) / (float)NumItemDistanceShade);
                        ...
                        glm::vec3 col = (i == ActiveSubMenu) ? ItemTextActive : ItemTextInactive;
                        Text::Render(SceneManager::Objects[i].GetName(), centerX  - MenuWidth / 2.0f + ItemPaddingXPX - xoffset,
                                     top_edge - titleHeight - itemHeightPX * (i + 1) + ItemPaddingYPX + yoffset, ItemTextScaling, col);
                    }

                    // Selection Marker
                    DrawRect(glm::vec2(right_edge - xoffset, top_edge - titleHeight - itemHeightPX * (ActiveSubMenu) + yoffset),
                            glm::vec2(right_edge - ItemSelectionThickness - xoffset, top_edge - titleHeight - itemHeightPX * (ActiveSubMenu + 1) + yoffset), ThemeColor);
                }
            ...
            else
            {
                ...
                DrawRect(glm::vec2(right_edge - xoffset, top_edge),
                         glm::vec2(left_edge  - xoffset, top_edge - titleHeight), ThemeColor);
                Text::Render(Title, centerX - Text::CalculateTextWidth(Title, TitleBarTextScaling) / 2.0f - xoffset,
                                    top_edge - titleHeight + TitleBarPaddingYPX, TitleBarTextScaling, TitleBarTextColor);

                for (int i = 0; i < subMenus.size(); i++)
                {    
                    float distance  = ((subMenus.size() - glm::abs(ActiveSubMenu - i)) / (float)subMenus.size());
                    float itemShade = distance * distance * distance;
                    itemShade = glm::pow(itemShade, 1.0f / 2.2f);
                    ...
                    Text::Render(subMenus[i].Title, centerX  - MenuWidth / 2.0f + ItemPaddingXPX - xoffset,
                                 top_edge - titleHeight - itemHeightPX * (i + 1) + ItemPaddingYPX, ItemTextScaling, col);
                }

                // Selection Marker
                if (Title == mainMenu.Title)
                {
                    if (inSubMenu)
                    {
                        DrawArrow(glm::vec2(right_edge - xoffset, top_edge - titleHeight - itemHeightPX * (ActiveSubMenu)),
                                  glm::vec2(right_edge - ItemSelectionThickness - xoffset,  top_edge - titleHeight - itemHeightPX * (ActiveSubMenu + 1)), ThemeColor);
                    }
                    else DrawRect(glm::vec2(right_edge - xoffset, top_edge - titleHeight - itemHeightPX * (ActiveSubMenu)),
                                  glm::vec2(right_edge - ItemSelectionThickness - xoffset,  top_edge - titleHeight - itemHeightPX * (ActiveSubMenu + 1)), ThemeColor);
                }
                else
                {
                    DrawRect(glm::vec2(right_edge - xoffset, top_edge - titleHeight - itemHeightPX * (ActiveSubMenu)),
                             glm::vec2(right_edge - ItemSelectionThickness - xoffset,  top_edge - titleHeight - itemHeightPX * (ActiveSubMenu + 1)), ThemeColor);
                }
            }
        }
    ...
    void WindowResize()
    {
        itemHeightPX = Text::CalculateMaxTextHeight("Scene Manager", ItemTextScaling, true) + ItemPaddingYPX * 2;
